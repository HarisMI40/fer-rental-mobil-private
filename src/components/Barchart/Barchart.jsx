...
function Barchart() {
    ...
    const  chartReport  = useSelector(state => {return state.order});
    ...
    useEffect(() => {
        dispatch(chartOrderReport(params))
    ...
    return (
      <div className="h-full mt-10 w-full">
        ...
        <div className="flex items-centers">
          <select className="w-46 h-9 cursor-pointer form-select" id="inputGroupSelect04"  value={dropdownValue} onChange={(e) => {handleSetParams(e.target.value); setDropdownValue(e.target.value)}}>
          ...
          <button className="blue-btn hover:bg-blue-700" type="button" onClick={() => dispatch(chartOrderReport(params))}>Go</button>
        </div>
        {
            !loading ? <Bar 
            options={options} data={data} /> : <h1>Loading...</h1>
        }
      </div>
    )
}
